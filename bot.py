import os
import asyncio
import math
import logging
import mimetypes
import time
from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkup
from telegram.ext import Application, CommandHandler, MessageHandler, filters, ContextTypes
from pyrogram import Client
from aiohttp import web

# --- CONFIGURATION ---
API_ID = int(os.environ.get("API_ID", "12345")) 
API_HASH = os.environ.get("API_HASH", "your_api_hash")
BOT_TOKEN = os.environ.get("BOT_TOKEN", "your_bot_token")
CHANNEL_ID = int(os.environ.get("CHANNEL_ID", "-100123456789")) 
PORT = int(os.environ.get("PORT", "8080"))
HOST_URL = os.environ.get("HOST_URL", "https://your-app.koyeb.app")

# Logging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

# --- PYROGRAM CLIENT (BACKEND) ---
# Multi-threaded download workers badha diye hain fast speed ke liye
pyro_client = Client(
    "stream_bot_session",
    api_id=API_ID,
    api_hash=API_HASH,
    bot_token=BOT_TOKEN,
    no_updates=True,
    max_concurrent_transmissions=5 
)

# --- HELPER FUNCTIONS ---
def get_readable_size(size):
    if size == 0: return "0B"
    units = ("B", "KB", "MB", "GB", "TB")
    i = int(math.floor(math.log(size, 1024)))
    p = math.pow(1024, i)
    s = round(size / p, 2)
    return f"{s} {units[i]}"

async def get_file_properties(message):
    media = message.video or message.document or message.audio
    if not media: return None
    
    file_name = getattr(media, "file_name", "video.mp4")
    file_size = getattr(media, "file_size", 0)
    mime_type = getattr(media, "mime_type", mimetypes.guess_type(file_name)[0] or "application/octet-stream")
    
    return media, file_name, file_size, mime_type

# --- BOT COMMAND HANDLERS ---
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "👋 **Video Stream Bot Ready!**\n\nFile bhejo, main Playable Link dunga.",
        parse_mode="Markdown"
    )

async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = update.message
    if not (msg.document or msg.video or msg.audio): return

    status_msg = await msg.reply_text("🔄 **Processing...**")

    try:
        # File forward to Bin Channel
        forwarded_msg = await msg.forward(chat_id=CHANNEL_ID)
        msg_id = forwarded_msg.message_id
        
        # Proper URL generation
        stream_link = f"{HOST_URL}/watch/{msg_id}"
        download_link = f"{HOST_URL}/download/{msg_id}"

        # Get File Info for display
        file_name = msg.document.file_name if msg.document else (msg.video.file_name if msg.video else "file")
        
        text = (
            "✅ **Link Generated!**\n\n"
            f"📂 **File:** `{file_name}`\n"
            f"🔗 **Stream Link:** [Click to Play]({stream_link})\n"
            f"📥 **Download Link:** [Click to Download]({download_link})"
        )
        
        kb = [[InlineKeyboardButton("▶️ Play Video", url=stream_link)]]
        
        await status_msg.edit_text(text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(kb))

    except Exception as e:
        logger.error(f"Error: {e}")
        await status_msg.edit_text(f"❌ Error: {str(e)}")

# --- STREAMING ENGINE (THE MAIN FIX) ---

class Streamer:
    # Custom iterator to handle Pyrogram chunks
    def __init__(self, client, message, offset, length):
        self.client = client
        self.message = message
        self.offset = offset
        self.length = length

    def __aiter__(self):
        return self

    async def __anext__(self):
        # Pyrogram se chunks stream karna
        # Note: Yeh complex part hai, hum direct pyrogram ka stream use kar rahe hain
        # lekin aiohttp ke response format mein
        try:
            # Hum yahan chunks generate karte hain
            async for chunk in self.client.stream_media(
                self.message,
                offset=self.offset,
                limit=self.length
            ):
                return chunk
            raise StopAsyncIteration
        except Exception:
            raise StopAsyncIteration

async def media_streamer(request, mode="inline"):
    try:
        message_id = int(request.match_info['message_id'])
        msg = await pyro_client.get_messages(CHANNEL_ID, message_id)
        
        if msg.empty or not (msg.video or msg.document or msg.audio):
            return web.Response(status=404, text="File not found")

        media, file_name, file_size, mime_type = await get_file_properties(msg)
        
        # --- RANGE HEADER HANDLING (Seeking Fix) ---
        range_header = request.headers.get("Range")
        
        if range_header:
            from_bytes, until_bytes = range_header.replace("bytes=", "").split("-")
            from_bytes = int(from_bytes)
            until_bytes = int(until_bytes) if until_bytes else file_size - 1
            content_length = (until_bytes - from_bytes) + 1
            
            headers = {
                "Content-Type": mime_type,
                "Content-Range": f"bytes {from_bytes}-{until_bytes}/{file_size}",
                "Content-Length": str(content_length),
                "Content-Disposition": f'{mode}; filename="{file_name}"',
                "Accept-Ranges": "bytes",
            }
            
            # Pyrogram stream generator
            generator = pyro_client.stream_media(msg, offset=from_bytes, limit=content_length)
            
            return web.Response(
                body=generator,
                status=206, # Partial Content (Video Playing ke liye zaroori)
                headers=headers
            )
        else:
            # Full Download Request
            headers = {
                "Content-Type": mime_type,
                "Content-Length": str(file_size),
                "Content-Disposition": f'{mode}; filename="{file_name}"',
                "Accept-Ranges": "bytes",
            }
            generator = pyro_client.stream_media(msg)
            return web.Response(
                body=generator,
                status=200,
                headers=headers
            )

    except Exception as e:
        logger.error(f"Stream Error: {e}")
        return web.Response(status=500, text="Server Error")

async def watch_handler(request):
    # Mode = "inline" (Browser mein play hoga)
    return await media_streamer(request, mode="inline")

async def download_handler(request):
    # Mode = "attachment" (Force download hoga)
    return await media_streamer(request, mode="attachment")

async def health_check(request):
    return web.Response(text="Bot Alive", status=200)

# --- MAIN LOOP ---
async def main():
    await pyro_client.start()
    print("✅ Pyrogram Client Started")

    ptb_app = Application.builder().token(BOT_TOKEN).build()
    ptb_app.add_handler(CommandHandler("start", start))
    ptb_app.add_handler(MessageHandler(filters.Document.ALL | filters.VIDEO | filters.AUDIO, handle_file))
    
    await ptb_app.initialize()
    await ptb_app.start()
    await ptb_app.updater.start_polling()
    print("✅ Bot Polling Started")

    app = web.Application()
    app.router.add_get("/", health_check)
    app.router.add_get("/watch/{message_id}", watch_handler)    # Play Link
    app.router.add_get("/download/{message_id}", download_handler) # Download Link

    runner = web.AppRunner(app)
    await runner.setup()
    site = web.TCPSite(runner, "0.0.0.0", PORT)
    await site.start()
    print(f"✅ Server running on Port {PORT}")

    stop_event = asyncio.Event()
    await stop_event.wait()

if __name__ == "__main__":
    asyncio.run(main())
